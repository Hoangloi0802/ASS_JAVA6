        ...
        public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
                http
                                ...
                                .authorizeHttpRequests(auth -> auth
                                                .requestMatchers("/Dangnhap", "/profile", "/donmua", "/donhang",
                                                                "/chitietdonhang", "/shop",
                                                                "/", "/Dangky", "/css/**", "/js/**", "/img/**",
                                                                "/bootstrap-5.3.3/dist/**", "/fonts/**",
                                                                "/giohang", "/chitiet", "/thanhtoan",
                                                                "/admin/categories/**", "/admin/account/**",
                                                                "/admin/products/**", "/admin/bill/**",
                                                                "/admin/statistics/**", "/checkotp", "/quenmk",
                                                                "/datlaimk", "/admin/nhansu")
                                                .permitAll() // Không cần đăng nhập
                                                .requestMatchers("/admin/**").hasAuthority("ROLE_ADMIN") // Chỉ Admin
                                                                                                         // truy cập
                                                                                                         // được
                                                .requestMatchers("/user/**").hasAuthority("ROLE_USER") // Chỉ User truy
                                                                                                       // cập được
